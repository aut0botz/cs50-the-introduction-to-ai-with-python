...
class MinesweeperAI():
            ...
            for col in range(cell[1] - 1, cell[1] + 2):
                if (row, col) != cell:
                    if (row, col) in self.mines:
                        count -= 1
                    elif (row, col) not in self.safes and 0 <= row < self.height and 0 <= col < self.width:
                        cells.add((row, col))
        if len(cells) > 0:
            sentence_new = Sentence(cells, count)
        ...
        for sentence1 in self.knowledge:
            for sentence2 in self.knowledge:
                ...
                if sentence1.cells.issuperset(sentence2.cells):
                    ...
                    if new_sentence not in knowledge_copy and len(new_cells) > 0:
                        knowledge_copy.append(new_sentence)
    ...
    def make_random_move(self):
        ...
        if len(all_moves) == 0:
            return None
        return random.choice(tuple(all_moves))
